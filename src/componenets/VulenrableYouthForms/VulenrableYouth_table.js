...
const List = props => (
    ...
    <tr>
        ...
        <td>
            <Link to={"/Vulnerable-youth/edit/" + props.data._id} style={{ marginRight: 4 }}> <span className="glyphicon glyphicon-edit"></span>Edit</Link>
            <Link to={"/Vulnerable-youth/delete/" + props.data._id} style={{ marginLeft: 4, color: "red" }} ><span className="glyphicon glyphicon-trash"></span>Delete</Link>
        </td>
    </tr>
);
...
export default class Mental_table extends React.Component {
    ...
    componentDidMount() {
        const database = new Datastore("../Vulnerable-youth.db");
    ...
    DataList = () => {
        counter = 1;
        return this.state.Vulnerable_youth_Data.map((current, index, counter) => {
