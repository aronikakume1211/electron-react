...
let database, globalId, globalkey, wor, zo;
export default class Hiv_Addis extends React.Component {
  ...
  LoadData = (limit) => {
    database = new DataStore('../Hiv_Adis_data.db');
  ...
  SearchResult = () => {
    ...
    if (this.state.kebeleSearch == '') {
      this.setState({
        Hiv_Adis_Data: this.state.temp
      })
    } else if (this.state.kebeleSearch == 'All' && this.state.woredaSearch == 'All') {
      console.log(this.state.woredaSearch);

      result = this.state.temp.filter(item => item.Kebele == this.state.kebeleSearch)
      if (result.length > 0) {
        this.setState({
          Hiv_Adis_Data: result,
          kebeleSearch: ''
        })
      }
    } else {
      ...
      result = this.state.temp.filter(item => item.Zone == zo)
  ...
  DeleteItem = (id) => {
    if (window.confirm("እርግጠኛ ንዎት ይጥፋ?")) {
      const items = this.state.Hiv_Adis_Data.filter(item => item._id !== id)
      this.setState({
        Hiv_Adis_Data: items
      });
  ...
  DataList = () => {
    let counter = 1;
    return this.state.Hiv_Adis_Data.map((current, index) => {
      return (
        <tr key={index}>
          <td>{counter++}</td>
  ...
  onSubmit = (e) => {
    ...
    if (this.state.fname == '' || this.state.lname == '' ||
      ...
      if (this.state.Age < 0) {
        ...
        //update from array
        let arr = this.state.Hiv_Adis_Data;
