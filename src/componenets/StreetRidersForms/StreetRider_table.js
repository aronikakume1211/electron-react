...
export default class Street_Riders_table extends Component {
    ...
    LoadData = () => {
        database = new DataStore("../StreetRider_Data.db");
        ...
        database.find({})
            ...
            .exec((err, docs) => {
                if (err) console.log(err)
                    ...
                    this.setState({
                        StreetRider_Data: docs
                    });
                    this.setState({
                        temp: this.state.StreetRider_Data
                    })
                }
            })
    }
    ...
    SearchResult = () => {
        ...
        if (wor == 'All' && zon == 'All' && keb == '') {
            this.setState({
                StreetRider_Data: this.state.temp
            })
    ...
    DeleteItem = (id) => {
        if (window.confirm("እርግጠኛ ንዎት ይጥፋ?")) {
            const items = this.state.StreetRider_Data.filter(item => item._id !== id)
            this.setState({
                StreetRider_Data: items,
                temp:items
            });
    ...
    DataList = () => {
        let counter = 1;
        return this.state.StreetRider_Data.map((current, index) => {
            return (
                <tr key={index}>
                    <td>{counter++}</td>
    ...
    onSubmit = (e) => {
        ...
        if (fname == '' || lname == '' || mname == '' || Age == '') {
            ...
            if (Age < 0) {
                ...
                let arr = this.state.StreetRider_Data;
                arr[globalkey] = UpdateData;
                this.setState({
                    StreetRider_Data: arr,
                    temp: arr,
                    isEditing: false
                })
