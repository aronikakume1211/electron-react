...
export default class Mental_table extends React.Component {
    ...
    LoadData = () => {
        database = new DataStore("../Mental_disorder_data.db");
    ...
    DeleteItem = (id) => {
        if (window.confirm("እርግጠኛ ንዎት ይጥፋ?")) {
            const items = this.state.Mental_Data.filter(item => item._id !== id)
            this.setState({
                Mental_Data: items
            });
    ...
    DataList = () => {
        let counter = 1;
        return this.state.Mental_Data.map((current, index) => {
            return (
                <tr key={index}>
                    <td>{counter++}</td>
    ...
    onSubmit = (e) => {
        ...
        if (fname == '' || lname == '' || mname == '' ||
            ...
            if (Age < 0) {
                ...
                let arr = this.state.Mental_Data;
